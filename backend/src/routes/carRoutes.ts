...
routes.route(routeNames.addImages).post([authJwt.verifyToken, authJwt.authSupplier, multer({ storage: multer.memoryStorage() }).array('images', 20)], carController.addImages)
routes.route(routeNames.deleteCarImage).post(authJwt.verifyToken, authJwt.authSupplier, carController.deleteCarImageFromList)
routes.route(routeNames.reorderImages).put(authJwt.verifyToken, authJwt.authSupplier, carController.reorderImages)
