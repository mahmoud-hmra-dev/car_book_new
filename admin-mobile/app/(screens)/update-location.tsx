...
const UpdateLocation = () => {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        if (id) {
          const _location = await LocationService.getLocation(id, language)
          setLocation(_location)

          const names: Record<string, string> = {}
          if (_location.values) {
            for (const v of _location.values) {
              names[v.language] = v.value || ''
            }
          }
          setNamesByLang(names)
          setLatitude(_location.latitude != null ? String(_location.latitude) : '')
          setLongitude(_location.longitude != null ? String(_location.longitude) : '')
        }
      } catch (err) {
        helper.error(err)
      } finally {
  ...
  const handleSave = async () => {
    try {
      ...
      const country = location.country as bookcarsTypes.Country | undefined
      ...
      const status = await LocationService.updateLocation(location._id, payload)
      if (status === 200) {
        helper.toast(i18n.t('LOCATION_UPDATED'))
        router.back()
      } else {
        helper.error()
      }
  ...
  if (!location) {
    return (
      <View style={styles.container}>
        <Header title={i18n.t('UPDATE_LOCATION')} loggedIn reload />
